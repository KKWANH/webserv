...
class CGIProcess {
	...
	public:
		...
		void setEnvp(HTTPData* data) {
			...
			//cgi루트에서 파일명만 뺴고 싶다면
			std::string root = this->data->CGI_root;
			...
			_envMap[std::string("CONTENT_TYPE")] = this->data->CGI_what;
//			_envMap[std::string("CONTENT_LENGTH")] = reqMsg->getHeaderField("Content-Length");
//			정말 헤더의 저것을 나타내는게 맞나?

			_envMap[std::string("REMOTE_ADDR")] = this->data->client_ip; // server socket addr
			_envMap[std::string("SERVER_PORT")] = this->data->server_port; // host port
			// 요부분은 RFC에 명시되어 있어서 해둬야 할듯
			_envMap[std::string("QUERY_STRING")] = this->data->qurey_string;
			_envMap[std::string("SCRIPT_NAME")] = this->data->CGI_root;
			_envMap[std::string("PATH_INFO")] = this->data->CGI_root;
//			cgi스크립트가 어디에 있는거지
//			저거 맞겠지?
			...
			_envMap[std::string("REQUEST_URI")] = this->data->CGI_root;
			_envMap[std::string("DOCUMENT_URI")] = this->data->CGI_root;
			// _envMap[std::string("SERVER_NAME")] = requestMessage->_serverName; // config 파일의 서버 이름
			_envMap[std::string("SCRIPT_FILENAME")] = this->data->CGI_root;
			//그 이외RFC에 있는데 안들어간거
