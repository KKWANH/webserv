...
#include "./../SocketController/SocketRun.hpp"
	...
	public:
		static int				serverProcess(SocketController* Socket, KernelQueueController* Kqueue) {
			while (true) {
				...
				// 적용된 event 초기화
				Kqueue->clearChangeList(Socket);
				Kqueue->resetChangeCount();


				// queue내 남아있는 이벤트만큼 반복
				for (int i = 0; i < Kqueue->getPollingCount(); i++) {
					SocketRun *udata = reinterpret_cast<SocketRun*>(Kqueue->getUdata(i));
					...
					if (Kqueue->getEventList(i)->filter == EVFILT_READ) {
						...
						// client read
							...
							if (n == -1) {
								...
								Kqueue->setWriteKqueue(fd, Socket);
